...
export function buildCompanion(type, colors) {
  ...
  if (type === 'lysrev') {
    ...
    snout.position.set(0.36, 0.2, 0);
    bob.add(snout);
    earL = new THREE.Mesh(new THREE.ConeGeometry(0.055, 0.13, 8), bodyMat);
    earL.position.set(0.2, 0.36, 0.07); bob.add(earL);
    earR = earL.clone(); earR.position.z = -0.07; bob.add(earR);
    tail = new THREE.Group(); tail.position.set(-0.22, 0.2, 0); bob.add(tail);
    ...
    eyeL.position.set(0.32, 0.26, 0.06); bob.add(eyeL);
    ...
    body.position.y = 0.2; bob.add(body);
    const head = new THREE.Mesh(new THREE.SphereGeometry(0.14, 12, 10), bodyMat);
    head.position.set(0.2, 0.28, 0); bob.add(head);
    earL = new THREE.Mesh(new THREE.ConeGeometry(0.05, 0.09, 6), bodyMat);
    earL.position.set(0.16, 0.4, 0.07); bob.add(earL);
    earR = earL.clone(); earR.position.z = -0.07; bob.add(earR);
    tail = new THREE.Group(); tail.position.set(-0.2, 0.22, 0); bob.add(tail);
    ...
    for (let i = 0; i < 3; i++) {
      ...
      puff.position.set(-0.1 + i * 0.1, 0.42 + (i % 2) * 0.03, 0);
      bob.add(puff);
    }
    const eyeL = new THREE.Mesh(new THREE.SphereGeometry(0.02, 6, 6), eyeMat);
    eyeL.position.set(0.3, 0.3, 0.06); bob.add(eyeL);
    const eyeR = eyeL.clone(); eyeR.position.z = -0.06; bob.add(eyeR);
    ...
    head.position.set(0.26, 0.26, 0); bob.add(head);
    earL = new THREE.Mesh(new THREE.ConeGeometry(0.055, 0.14, 8), bodyMat);
    earL.position.set(0.22, 0.4, 0.06); bob.add(earL);
    earR = earL.clone(); earR.position.z = -0.06; bob.add(earR);
    tail = new THREE.Group(); tail.position.set(-0.24, 0.22, 0); bob.add(tail);
    const tailMesh = new THREE.Mesh(new THREE.CapsuleGeometry(0.07, 0.24, 2, 8), bodyMat);
    tailMesh.rotation.z = Math.PI / 3;
    tail.add(tailMesh);
    const eyeL = new THREE.Mesh(new THREE.SphereGeometry(0.02, 6, 6), eyeMat);
    eyeL.position.set(0.35, 0.28, 0.06); bob.add(eyeL);
    const eyeR = eyeL.clone(); eyeR.position.z = -0.06; bob.add(eyeR);
  } else {
    // stjernedrage
    movement = 'float';
  ...
  return { group: root, bob, parts: { tail, earL, earR, wingL, wingR }, movement };
}
